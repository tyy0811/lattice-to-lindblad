...
**Day-14 narrative-template note for Task 16**: the original `top_3_sensitivities` renders the top-3 parameters by `\|S_θ\|` regardless of bar/point status. At REFERENCE the top-3 are `ε₀ (+0.050)`, `τ (+0.037)`, `chi_scale (−0.029)` — the third is noise-consistent. Task 16's narrative should render as "top-3 bar-rendered parameters" when not all top-3 are above the rendering threshold, so the narrative doesn't imply χ is a dominant lever at REFERENCE.
