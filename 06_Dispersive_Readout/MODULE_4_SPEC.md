...
### 0.4 Day-13 execution-time amendments (closed-loop demo + solver bug + two-peak physics)

| # | Execution-time amendment | Driver |
|---|---|---|
| 16 | **Pareto solver warm-start grid bug and multi-start SLSQP fix**: `find_pareto_point` shipped with a 5-point linear warm-start grid over the 3-decade `eps_0 ∈ [1e6, 1e9]` domain. Day-13 Task-17 picker diagnostic revealed every device converged to `eps_opt = 2.5075e8 exactly` (the n=5 warm-start winner; SLSQP unable to cross the inter-grid valley to the true basin at `eps ≈ 1.51e8`). All 30 Pareto points in Task 14's shipped cache (commit `e6d2e09`) reported `F_opt` values ~6 percentage points below truth. Root cause (verified via 50-point F(eps) scan at REFERENCE, tau=500ns): the F(eps_0, tau) surface at fixed tau has **two local maxima** — peak #1 at `eps = 7.76e7` (F = 0.99324), valley at `eps = 1.05e8` (F = 0.90853), global peak #2 at `eps = 1.51e8` (F = 0.99421) — separated by a sharp 0.08-deep F valley. SLSQP from any start outside a peak's basin is trapped at a local max; no gradient method crosses the valley. **Fix (multi-component)**: (a) warm-start grid topology changed to log-spaced on eps (linear on tau), n=10; (b) top-K=5 diverse candidates selected by F with eps-only symmetric-ratio separation filter (`min_eps_ratio=1.2`); (c) per-start sub-grid refinement before SLSQP (5-point linear eps sweep ±40%, tau fixed) pulls each start into the nearest local-max basin's interior; (d) zero-width integration window guard in `_F_analytic_at` (returns F=0.5 for `tau <= integration_window[0]` instead of propagating `ValueError`). Local verification at REFERENCE: F_opt=0.9938, eps_opt=1.40e8, tau_opt=500ns, matches the n=20 linear reference within shot noise. Test `test_warm_start_resolves_basin_at_reference` asserts F_opt ≥ 0.99 at REFERENCE, tau_max=500ns; regresses if any of the four components reverts. **Affected artifacts regenerated**: Task 14 Panel (c) cache, Task 17 picker YAML. | Execution finding caught by designing a downstream diagnostic (Day-13 closed-loop picker) rather than by adversarial review. Full diagnostic: `docs/module4_diagnostics/warm_start_grid_bug.md`. |
| 17 | **F(eps) two-peak structure and shared-argmax physics finding**: at the corrected solver, the F(eps_0, tau) surface at REFERENCE reveals two physically-distinct operating regimes: peak #1 (`eps ≈ 7.76e7`) is the low-photon regime where dispersive contrast is clean and shot-noise-limited; peak #2 (`eps ≈ 1.51e8`) is the dispersive-saturation regime where n̄_phot approaches the bifurcation threshold and contrast is decoherence-limited. Both peaks verified across 3 devices spanning T_1 ∈ [5.4, 91.9] µs from the SEED=42 recovery harness (low_T_1 idx=18, high_T_1 idx=41), with identical peak locations. Peak #2 is consistently global across the T_1 span by ~0.001 F margin. **Pareto curve consequence**: Panel (c) frontiers transition between the two regimes around `tau_max ≈ 450 ns` — solver picks `eps ≈ 8e7` (peak #1) at tau_max ∈ [100, 379] ns (short integration prefers low-photon cleanness), and `eps ≈ 1.4e8` (peak #2) at tau_max ∈ [528, 2000] ns (long integration prefers saturation-SNR). **Closed-loop demo consequence**: for the harness (κ, g, omega_r inherited from REFERENCE, only decoherence+omega_q varied), the Pareto argmax location is decoherence-invariant — all 50 harness devices converge to `eps_opt = 1.4e8, tau_opt = 500 ns` **bit-identically** at tau_max=500ns, differing only in F (range 0.9924-0.9947, σ=3.86e-4). Picker selection criterion extended with shared-argmax fallback: when max drift < 1e-4 relative, pick by minimum F (hardest-decoherence device). Selected demo device idx=18 (T_1=5.4 µs, T_2_echo=6.5 µs, omega_q/(2π)=4.7224 GHz, F_opt=0.9924), same device used in the peak-ordering verification. **O5 bifurcation**: O5a/O5b each split into -shift (assert improvement > 0.005 / Welch-t p < 0.05 when drive differs > 5% from REFERENCE) and -confirm (assert consistency / indistinguishability within shot noise when drive shared). Gate evaluated at test-collection time from `closed_loop_demo_device.yaml`; exactly one variant per test fires per run, skipif on the other with an explicit reason. | Execution finding surfaced during multi-start solver implementation (item 16). Quantitatively supports the Q3 / Panel (a) caption observation that Marxer's REFERENCE χ is tuned for measurement-induced-transition suppression alongside F_assign: peak #1 exists as a lower-photon alternative operating point that a pure-F optimizer would not find. The two-peak substructure is a direct numerical consequence of the per-level `χ_j` asymmetry (Amendment #15). |

**Caption language for Figure 4 Panel (c) (Day-13 lock).** *"Pareto frontiers transition between two dispersive-readout operating regimes around τ_max ≈ 450 ns: the low-photon regime (peak #1, ε_0 ≈ 8×10⁷ rad/s — dispersive-shift clean, shot-noise-limited contrast) optimal at short integration, and the dispersive-saturation regime (peak #2, ε_0 ≈ 1.4×10⁸ rad/s — n̄_phot near bifurcation threshold, decoherence-limited integration) optimal at long integration. Both regimes are visible as separate branches on the REFERENCE curve; the specific transition τ_max is determined by the trade-off between shot-noise-limited contrast (peak #1) and decoherence-limited integration (peak #2). Closed-loop scope: demo device idx=18 from SEED=42 recovery harness (T_1=5.4 µs, hardest-recovery candidate); pipeline confirms REFERENCE drive is near-optimal across the harness, reporting F_opt=0.9924 with visible decoherence penalty vs REFERENCE (ΔF = −0.0015 ≈ −sigma_shot at n_shots=10⁴). Full closed-loop including resonator spectroscopy and AC-Stark is post-submission roadmap (Q4 scope)."* **(Superseded by Day-14 Amendment #18 below — current rendering drops the ΔF annotation per the shared-argmax finding; §0.5.)**

### 0.5 Day-14 external-review amendments (rendering + physics-label corrections)

| # | Amendment | Driver |
|---|---|---|
| 18 | **Closed-loop demo rendered as a static recommended-point marker, not a default→optimized ΔF arrow.** In the closed-loop harness, fitted devices vary (T_1, T_2, ω_q) over fixed REFERENCE resonator/coupling. Per Amendment #17 the Pareto argmax is shared across the harness — `(ε_0_opt, τ_opt)` for the fitted device coincides bit-identically with REFERENCE's — so a default-to-optimized arrow would have zero horizontal extent and collapses to the F-axis decoherence penalty (ΔF = −0.0015 vs REFERENCE). An external review flagged that rendering this as a negative-ΔF arrow reads as an optimization regression to an application-audience reader. Rendering decision: show the demo device as an open marker at its recommended Pareto point with a neutral label ("closed-loop demo: fitted T_1=5.4 µs"), no ΔF annotation, no REFERENCE comparison on-panel. **No positive ΔF arrow is shown because the fitted/default and optimized control coordinates coincide in this scoped harness.** The Day-13 caption language above is superseded; the Day-14 caption (§7) leads with "open marker shows fitted synthetic demo device mapped by the closed-loop recommendation pipeline to its Pareto operating point." Closed-loop scope caveat (fitted T_1/T_2/ω_q over fixed REFERENCE resonator/coupling; full resonator spectroscopy post-submission) renders as a separate italic paragraph below the figure. | External reviewer flagged that the ΔF = −0.0015 arrow reads as "optimization made things worse" to a first-pass recruiter audience — technically honest but portfolio-hostile. Option C (break shared-argmax by varying κ or g) rejected because it expands the harness beyond Module 3's fitted-parameter scope (T_1, T_2, ω_q only; κ/g/ω_r are Q4-locked as post-submission). Option A (keep arrow, defend in caption) rejected because the minus sign is visible before the defense. |
| 19 | **`dispersive_breakdown_boundary` → `chi_phase_integration_boundary`.** Previous function name + legend label "Dispersive breakdown (χ/κ ≲ 1)" conflated a χ·τ timescale condition with the dispersive-approximation-validity boundary. The function computes `χ · τ_readout = 2π` (phase integration), not a χ/κ ratio; the dispersive-approximation-validity boundary is `g/Δ << 1` or `n̄_phot/n_crit << 1`, which is not plotted. New legend: "χ-phase accumulation (χ·τ ≈ 2π)". Docstring + §3.2 item 2 + §4 code surface updated; `MODULE_4_PLAN.md` retained as historical execution trail. Function behavior and plotted line unchanged; only label/name corrected. | Same external review. Physics-accuracy correction, not behavior change. |
...
2. **χ-phase integration** — locus where `χ · τ_readout = 2π`, i.e., the differential dispersive-shift phase between |g⟩ and |e⟩ accumulates one full 2π cycle within the readout window. With `χ = x · κ_REF` (holding κ at REFERENCE to resolve x-axis) and `τ_readout = 2π / χ(x)`, the y-axis coordinate is `y_disp(x) = γ_1_REF · 2π / (x · κ_REF)`. Above the line, phase-wrap periodicity degrades IQ-plane discrimination; below the line, only a fraction of the χ-phase is integrated. **Day-14 review correction:** previously named `dispersive_breakdown_boundary` with legend "Dispersive breakdown (χ/κ ≲ 1)" — that label conflated a χ·τ timescale condition with the dispersive-approximation-validity boundary (the latter would be `g/Δ << 1` or `n̄_phot/n_crit << 1`, not plotted here). Renamed to `chi_phase_integration_boundary` with legend "χ-phase accumulation (χ·τ ≈ 2π)" for physics accuracy. Function behavior and plotted line are unchanged.
...
def chi_phase_integration_boundary(...) -> np.ndarray: ...
