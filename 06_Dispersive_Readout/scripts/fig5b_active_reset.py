...
def main() -> None:
    ...
    ax_a.plot(tau_us, p_passive, 'k--', label='passive baseline')
    ax_a.plot(tau_us, p_active_ideal, 'C0-',
              label=r'active reset, $\varepsilon_X = 0$ (ideal gate)')
    ax_a.plot(tau_us, p_active_realistic, 'C1-',
              label=fr'active reset, $\varepsilon_X = {eps_X_5a:.2e}$ (5a-realistic)')
    ax_a.fill_between(
        tau_us,
    ...
    ax_a.axvline(tau_b * 1e6, color='gray', linestyle=':', alpha=0.5,
                 label=fr'panel-(b) point ({regime_label})')
    ax_a.set_xlabel(r'$\tau_{\rm meas}$ ($\mu$s)')
    ax_a.set_ylabel(r"$p_e'$ (excited residual after one cycle)")
    ax_a.set_yscale('log')
    ax_a.set_title(r'Active vs passive reset, idx=18 ($T_1 = 5.35\,\mu s$)')
    ax_a.legend(fontsize=8, loc='lower right')
    ax_a.grid(alpha=0.3)

    # Panel (b): stacked decomposition (largest-to-smallest)
    ordered = sorted(contributions.items(), key=lambda kv: -kv[1])
    labels = [k for k, _ in ordered]
    values = [v for _, v in ordered]
    ses = [contribution_se[k] for k in labels]
    cumulative = 0.0
    colors = {
        'missed_excited': 'C2',
        'false_positive_decayed': 'C3',
        'gate_failure_excited': 'C4',
    }
    pretty = {
        'missed_excited': 'missed-excited',
        'false_positive_decayed': 'false-positive (decayed)',
        'gate_failure_excited': fr'gate failure ($\varepsilon_X = {eps_X_5a:.2e}$)',
    }
    for label, value, se in zip(labels, values, ses):
        ax_b.bar(
            0, value, bottom=cumulative,
            color=colors[label],
            yerr=se, error_kw=dict(capsize=4, elinewidth=1, color='black'),
            label=fr'{pretty[label]} ({relative_pct[label]:.1f}%)',
        )
        cumulative += value
    ax_b.set_xticks([])
    ax_b.set_ylabel(r"contribution to $p_e'$")
    ax_b.set_title(
        fr'Residual decomposition at $\tau_{{\rm meas}} = {tau_b * 1e6:.2f}\,\mu s$ '
        fr'(regime: {regime_label})'
    )
