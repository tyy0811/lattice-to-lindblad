...
**v0 zero-temperature invariant (post-review enforcement).** `extract_joint_matrix` hard-codes the $s_i = 0$ branch to remain $|g\rangle$ for the entire measurement window. `physics/lindblad.py` *does* model thermal $|g\rangle \to |e\rangle$ excitation when `n_th > 0`, so any mixed-temperature device would silently drop a transition pathway the underlying simulator includes. After the adversarial-review correction:

- `device_idx18` overrides `n_th` to **exactly 0.0** (REFERENCE\_DEVICE has 0.01); the override is at the device-construction boundary, not a permissive `< 0.05` threshold.
- `extract_joint_matrix` raises `NotImplementedError` if any caller passes `n_th > 0`. v1.5 must add thermal sampling before this guard is relaxed.

Symmetrically, `analysis/reset_metrics.active_advantage` now scales the passive baseline by the prior `p_e` (i.e., `p_e · exp(-(τ_meas + τ_gate)/T₁)`), since at zero temperature the $(1 - p_e)$ ground-state weight contributes exactly 0 to passive residual. Without this correction, `active_advantage` could publish a phantom positive advantage at `p_e < 1` that was purely an artifact of the unweighted baseline. The fix preserves the worst-case `p_e = 1` figure unchanged.
...
- Thermal initial-state preparation ($\bar n_q > 0$): samples thermal excitation events from $|g\rangle$ as well as $T_1$ events from $|e\rangle$. v0 enforces strict $\bar n_q = 0$ (`device_idx18` override + `extract_joint_matrix` defensive guard, both added post-adversarial-review); v1.5 must lift this by adding the symmetric thermal-excitation channel and weighting the passive-baseline ground-state branch by the thermal floor. Estimated +0.5 day.
...
4. A post-implementation adversarial review surfaced two high-severity gaps that the validation matrix had missed: (a) `active_advantage` was comparing the active residual at any prior $p_e$ against the unweighted passive baseline, which would publish a phantom positive advantage at $p_e < 1$; and (b) `extract_joint_matrix` accepted devices with `n_th > 0` while hard-coding |g⟩-stays-|g⟩, silently dropping the thermal $|g\rangle \to |e\rangle$ pathway that `build_collapse_operators` *does* model. Both were fixed in-place: `active_advantage` now scales the passive baseline by $p_e$, `device_idx18` sets `n_th = 0` explicitly, and `extract_joint_matrix` raises if any caller still passes a thermal device. New tests (`test_active_advantage_scales_passive_baseline_by_prior` with a $p_e = 0$ sentinel; `test_device_idx18_zeroes_thermal_for_v0`; `test_extract_joint_matrix_rejects_thermal`) lock the corrected behavior in. The methodology lesson: prior-aware composition and cross-component invariant audits belong in the validation matrix, not bolted on after review.
