...
Output:
    ./research_highlight.pdf  (also copied to /mnt/user-data/outputs/)
...
IMG_MASSGAP  = os.path.join(IMG_DIR, "massgap_continuum(N=20).png")
...
def page3(c):
    ...
    ry2 = draw_body_text(c, rx2, ry2, (
        "The massless Schwinger model mass gap is extracted from "
        "sparse Lanczos diagonalisation across N=8\u201320 sites at "
        "8 lattice spacings (x=2\u201312). Finite-size control uses "
        "1/N extrapolation at intermediate spacings and conservative "
        "upper bounds at the finest lattices."
    ), desc2_w, size=7.5, leading=10.5)
    ry2 -= 6
    ry2 = draw_body_text(c, rx2, ry2, (
        "A weighted linear fit in (ag)<super>2</super> yields the "
        "continuum intercept M<sub>gap</sub>/g = 0.557, within "
        "<b>1.3%</b> of the exact analytic result "
        "1/\u221a\u03c0 \u2248 0.564. The fit is anchored by the "
        "most controlled data: the coarse stabilised point and the "
        "fine-lattice upper bounds, with intermediate extrapolated "
        "points appropriately downweighted. This demonstrates that "
        "the ED pipeline correctly captures continuum QFT physics "
        "from lattice Hamiltonian simulation."
    ), desc2_w, size=7.5, leading=10.5)
    ry2 -= 10

    kw2 = (desc2_w - 8) / 2;  kh2 = 38
    draw_kpi_box(c, rx2,           ry2 - kh2, kw2, kh2,
                 "Continuum extrapolation", "0.557", ACCENT_TEAL)
    draw_kpi_box(c, rx2 + kw2 + 8, ry2 - kh2, kw2, kh2,
                 "Exact result",            "0.564", ACCENT_BLUE)
...
def build():
    ...
    shutil.copy(out, "/mnt/user-data/outputs/research_highlight.pdf")
    print(f"Done: {out}")
